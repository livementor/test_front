...
export default {
  ...
  createMessage (store: Store<MessagesState>, payload: { conversationId: string, message: Message }) {
    ...
    store.commit('SET_MESSAGES', { conversationId: payload.conversationId, messages: [ref] })
  },
  ...
  async fetchMessagesForConversation (store: Store<MessagesState>, conversationId: string) {
    ...
    store.commit('conversations/SET_MESSAGES', { conversationId, messages: messagesRef.docs }, { root: true })
    store.commit('SET_MESSAGES', { messages: messagesRef.docs.map((message: any) => message.data()) })
  },
}
