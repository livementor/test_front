export default {
  ...
  createMessage (store, payload) {
    ...
    this.commit('conversations/SET_LAST_MESSAGE', { conversationId: payload.conversationId, message: payload.message })
  },
  ...
  async fetchLastMessagesFromConversation (_, conversationId) {
    const ref = await this.$fire.firestore.collection('conversations').doc(conversationId).collection('messages').orderBy('createdAt', 'desc').limit(1).get()
    this.commit('conversations/SET_LAST_MESSAGE', { conversationId, message: ref.docs.map(m => m.data())[0] })
  },
}
